...
export const getCountCategoryData = async () => await prisma.productCategories.count({
    where: {
...
export const getParent = async (idCategory: string) => await prisma.productCategories.findUnique({
    where: {
        idStore: store.get("store")?.value,
        id: idCategory
    },
